...
const calculateRelativeFrequency = (listIntervals) => {
...
const calculateInterval = (values, range) => {
  const listIntervals = [];
  let lastValue = values[0];
  console.log(values);
  for (let index = 1; index <= NUMBER_CLASS; index++) {
    const intervalMin = parseInt(lastValue);
    const intervalMax = parseInt(lastValue + range / NUMBER_CLASS);
...
const calculateClassMiddlePoint = (listIntervals) => {
  listIntervals.forEach((interval) => {
    interval.classMiddlePoint =
      (interval.values.max - 1 + interval.values.min) / 2;
  });
  return listIntervals;
};
...
const init = () => {
  ...
  const range = caculateRange(VALUES, NUMBER_CLASS);

  let listIntervals = calculateInterval(sortedValues, range);

  listIntervals = calculateAbsoluteFrequency(listIntervals, sortedValues);

  listIntervals = calculateRelativeFrequency(listIntervals);

  listIntervals = calculateCumulativeFrequency(listIntervals);

  listIntervals = calculateClassMiddlePoint(listIntervals);
  console.table(
    listIntervals.map((interval) => {
      return {
